...
def tset(   mf = None, ff = None, dr = None):
    ...
    if mf is None:
        mf = [name for name in os.listdir( p) if name[0] == 'm' or name[0] == 'M']
        mf = random.choice( mf)
    if ff is None:
        ff = [name for name in os.listdir( p) if name[0] == 'f' or name[0] == 'F']
        ff = random.choice( ff)
    #print ('dr%d/' % dr, mf, ff)
    ...
    ms = [lr.core.load(p+'/'+mf+'/'+n, sr=None)[0] for n in os.listdir( p+'/'+mf) if 'wav' in n.lower()]
    fs = [lr.core.load(p+'/'+ff+'/'+n, sr=None)[0] for n in os.listdir( p+'/'+ff) if 'wav' in n.lower()]
